...
This audit reviewed 18 UI/Compose files across the Milki Launcher codebase. The application demonstrates good architectural patterns (MVVM, unidirectional data flow) and a well-organized design system. However, several UX/UI improvements are needed, particularly around accessibility, error handling, and internationalization.
...
### 1. Missing Loading State UI
**File:** `AppSearchDialog.kt:153-169`  
**Severity:** Critical  

**Description:**  
The `SearchUiState` has an `isLoading` property that is tracked but never displayed to the user. When a search is in progress (especially for slower providers like file search), users see no feedback.

**User Impact:**  
Users may think the app is frozen or unresponsive when searching, leading to frustration and potential app abandonment.

**Suggested Improvement:**
```kotlin
// In AppSearchDialog.kt, after the search field
if (uiState.isLoading) {
    LinearProgressIndicator(
        modifier = Modifier.fillMaxWidth().padding(horizontal = Spacing.mediumLarge)
    )
}
```
...
**File:** `PinnedItem.kt:241` and `ItemActionMenu.kt:106-111`  
**Severity:** High  

**Description:**  
The "Unpin from home" action removes items immediately without confirmation. Users may accidentally unpin items and lose their arrangement.

**User Impact:**  
Accidental unpinning can disrupt user's carefully arranged home screen layout.

**Suggested Improvement:**
Add confirmation dialog or undo snackbar:
```kotlin
...
### 14. Missing Scroll Position Restoration
**File:** `SearchResultsList.kt`  
**Severity:** High  

**Description:**  
When switching between search providers (e.g., from app search to web search), scroll position is reset but not saved. If the user returns to the previous provider, they lose their scroll position.

**User Impact:**  
Users lose their place in long result lists when switching search modes.

**Suggested Improvement:**
Save scroll position per provider type using `saveableStateSaver`.
...
### 16. Inconsistent Icon Usage (Non-AutoMirrored)
**File:** `SettingsComponents.kt:279`  
**Severity:** Medium  

**Description:**  
Uses `Icons.Default.Check` instead of `Icons.AutoMirrored.Filled.Check` (if available) for the selected indicator.

**User Impact:**  
Minor: In RTL layouts, the check icon may not mirror appropriately.

**Suggested Improvement:**
Check if AutoMirrored version exists and use it.
