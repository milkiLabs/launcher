...
class HomeModelWriter(
    ...
    ): Result = updateTopLevelItem(currentItems, command.itemId) {
    ...
    private fun updateWidgetDisplayMode(
        currentItems: List<HomeItem>,
    ...
    private fun updateTopLevelItem(
        ...
        val existing = mutable.getOrNull(index) ?: return Result.Rejected(Error.ItemNotFound)

        val updated = modification(existing) ?: return Result.Rejected(Error.InvalidWidgetOperation)
        val span = updated.homeGridSpan
        val occupied = HomeGraph.buildOccupiedCells(mutable, excludeItemId = itemId)

        val rejection = when {
            !isWithinGrid(updated.position, span, gridColumns) -> Error.OutOfBounds
            !isSpanFree(updated.position, span, occupied) -> Error.TargetOccupied
            else -> null
        }
