...
fun DraggablePinnedItemsGrid(
    ...
    BoxWithConstraints(modifier = modifier.fillMaxSize()) {
        ...
        // Calculate the ideal height (25% taller than width for 2-line labels)
        val preferredCellHeightPx = cellWidthPx * 1.25f
        
        // Snap to the nearest integer number of rows to completely fill the screen
        // This eliminates the empty gap at the bottom and perfectly distributes the space
        val maxVisibleRows = kotlin.math.round(gridHeightPx / preferredCellHeightPx).toInt().coerceAtLeast(1)
        val cellHeightPx = gridHeightPx / maxVisibleRows
...
internal fun ExternalDragDropItem.toPreviewHomeItem(): HomeItem? {
    return when (this) {
        ...
        is ExternalDragItem.ActionShortcut -> shortcut
