...
class ExternalHomeDropActionTest {
    ...
    @Test
    fun resolveExternalDropAction_popupWidgetUsesSingleCellPlacementButKeepsWidgetSpan() {
        val providerInfo = AppWidgetProviderInfo().apply {
            provider = ComponentName("com.example.widgets", "ExampleWidgetProvider")
        }
        val action = resolveExternalDropAction(
        ...
        assertTrue(action is ExternalDropAction.PlaceWidget)
        val placeWidget = action as ExternalDropAction.PlaceWidget
        assertEquals(GridPosition(row = 0, column = 0), placeWidget.position)
        assertEquals(GridSpan(columns = 2, rows = 2), placeWidget.span)
        assertEquals(GridSpan.SINGLE, placeWidget.previewState.dragSpan)
        assertEquals(WidgetDisplayMode.PopupIcon, placeWidget.displayMode)
    }
